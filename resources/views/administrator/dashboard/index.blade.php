            ...
            <table class="w-full text-sm text-center rtl:text-right text-gray-500 dark:text-gray-400">
                <thead class="text-xs text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-3">SEMESTER GANJIL</th>
                        <th scope="col" class="px-6 py-3">SEMESTER GENAP</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach ($dataShow as $list)
                        <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200">
                            ...
                            <td class="px-6 py-4">{{ $list['nama'] }}</td>
                            <td class="px-6 py-4">{{ $list['kdkls'] }}</td>
                            <td class="px-6 py-4">
                                <div class="flex justify-center">
                                    <div class="flex gap-2">
                                        @if ($list['ganjil'])
                                            <a href="{{ route('admin.dashboard.statistik', ['siswa' => $list['nis'], 'semester' => 'ganjil']) }}"
                                            ...
                                            <button
                                                ...
                                                data-url="{{ route('statistik-page', ['siswa' => $list['nis'], 'semester' => 'ganjil']) }}">
                                                Bagikan
                                            </button>
                                        @else
                                            <div
                            ...
                            <td class="px-6 py-4">
                                <div class="flex justify-center">
                                    @if ($list['genap'])
                                        <a href="{{ route('admin.dashboard.statistik', ['siswa' => $list['nis'], 'semester' => 'genap']) }}"
                                            class="btn bg-blue-500 rounded-lg px-4 py-1 text-white w-fit cursor-pointer">
                                            Statistik Nilai
                                        </a>
                                        <button
                                            class="copyButton btn bg-blue-500 rounded-lg px-4 py-1 text-white w-fit cursor-pointer"
                                            data-url="{{ route('statistik-page', ['siswa' => $list['nis'], 'semester' => 'genap']) }}">
                                            Bagikan
                                        </button>
                                    @else
                                        <div
            ...
            $('.copyButton').click(function() {
                ...
                navigator.clipboard.writeText(url).then(() => {
                    Swal.fire({
                        icon: 'success',
                        title: 'Tersalin!',
                        text: 'Link URL statistik siswa disalin.',
                        timer: 1500,
