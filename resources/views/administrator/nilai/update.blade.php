                        ...
                        @foreach ($dataNilai as $list)
                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200">
                                <th scope="row"
                                    ...
                                    <input type="hidden" id="idmtpelajaran_{{ $list->idmtpelajaran }}"
                                        name="idmtpelajaran[{{ $list->idmtpelajaran }}]" value="{{ $list->idmtpelajaran }}">
                                    {{ $list->idMapel->nmmapel }}
                                </th>
                                <td class="px-6 py-4">
                                    <input type="number" id="nilai_{{ $list->idmtpelajaran }}"
                                        name="nilai[{{ $list->idmtpelajaran }}]" min="0" max="100"
                                        class="border shadow-sm rounded-md py-2 px-3 w-20 text-black"
                                        value="{{ $list->nilai ?? "" }}" required>
                                </td>
                                <td class="px-6 py-4">
                                    <textarea id="capaian_{{ $list->idmtpelajaran }}" name="capaian[{{ $list->idmtpelajaran }}]"
                                        class="border shadow-sm rounded-md py-2 px-3 w-full text-black">{{ $list->capaian ?? "" }}</textarea>
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
